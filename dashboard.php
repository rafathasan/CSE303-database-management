...
if( isset($_COOKIE['cuser_id']) && isset($_COOKIE['cpsw_hash']) ){
  ...
  if($keys!=null){
    foreach ($keys as $key) {
      setcookie($key, $user[0][$key], time() + (86400 * 30), $_SERVER['HTTP_HOST']);
    }
  }
...
if(isset($userName)&&isset($userGroup)&&isset($userModule)){
  ...
  <body class=\"w3-animate-bottom\" style=\"width:70%;margin:auto;\">
  ...
  if($userModule=="ALL" || $userModule=="INSERT"){
    ...
    <div class=\"w3-dropdown-hover\">
      <button class=\"w3-button\">Admission</button>
  ...
  if($userModule=="ALL" || $userModule=="VIEW"){
    echo "
    <div class=\"w3-dropdown-hover\">
      <button class=\"w3-button\">Data Analysis</button>
      <div class=\"w3-dropdown-content w3-bar-block w3-border\">
        <form  action=\"dashboard.php\" method=\"POST\">
  ...
  if($userModule=="ALL"&&$uname=='rafikhan'){
    echo "
    <form  action=\"dashboard.php\" method=\"POST\">
    <button class=\"w3-bar-item w3-button\" type=\"submit\" name=\"page\" value=\"userentry\">User Entry</button>
    </form>
    ";
  ...
  if(ISSET($_POST['page'])){
      ...
      }elseif ($page == 'userentry') {
        include("./html/userentry.html");
      }else{
  ...
  echo "<form id=\"myForm\" action=\"./index.php\" method=\"post\">
        <input type=\"hidden\" name=\"attempt\" value=\"false\">
    </form>
    <script type=\"text/javascript\">
        document.getElementById(\"myForm\").submit();
    </script>";
